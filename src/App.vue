  ...
  >
    <transition name="fade">
      <div class="splash-screen" v-if="splashScreen">
        <div class="wrap">
          <img src="/logo-cropped.png" class="logo" alt="logo" />
          <img src="/Ripple-2s-100px.svg" alt="loading-image" />
        </div>
      </div>
    </transition>
    ...
    <div class="container flex column box grow">
      ...
      <div class="main box grow flex">
        ...
        <transition :name="viewAnimation" mode="out-in">
          <router-view class="view box grow" :key="$route.fullPath" />
        </transition>
      </div>
...
export default {
  ...
  computed: {
    ...
    isLoading() {
      return this.$store.getters.isLoadingPage;
    }
  },
  methods: {
    resizeOpenNav() {
  ...
  mounted() {
    ...
    // this.$Progress.start()
    // setTimeout(() => {
    // 	this.$store.commit('isLoadingPage', false)
    // }, 10000)
