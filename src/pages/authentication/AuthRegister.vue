				...
				<el-row :gutter="30" class="no-p no-m">
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="tab-invert bg-primary p-30">
						...
						<template v-if="windowWidth>=992">
							...
							<div class="text-center" style="padding-bottom: 730px;">
					...
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="p-30">
						...
						<form action="" @submit.prevent="register">
							<!-- <float-label class="styled" v-if="source_register!=''">
								<input required type="text" v-model="source_register" disabled>
							</float-label> -->
							...
							<float-label class="styled">
								<input required type="email" v-model="form.email" placeholder="Company Business Email">
							</float-label>
							<float-label class="styled">
							...
							<float-label class="styled">
								<label style="font-size: 14px;">Where did you hear about us?</label><br>
								<el-select v-model="form.option_hear" placeholder="Where did you hear?" style="width: 100%;">
									<el-option
										v-for="item in optionsHear"
										:key="item.value"
										:label="item.label"
										:value="item.value">
									</el-option>
							...
							<float-label class="styled">
								...
								<!-- <el-select required v-model="form.question1" placeholder="Select answer" style="width: 100%;">
									<el-option
							...
							<float-label class="styled">
								...
								<!-- <el-select required v-model="form.question2" placeholder="Select answer" style="width: 100%;">
									<el-option
										v-for="item in optionsQuestion"
										:key="item.value"
										:label="item.label"
...
export default {
	...
	data() {
		return {
			...
			optionsHear: [{
				...
				value: 'Facebook Partner Discovery',
				label: 'Facebook Partner Discovery'
				}, {
				value: 'Facebook',
				label: 'Facebook'
				}, {
	...
	methods: {
		...
		async register() {
			...
			await api({
				...
				data: { 
					source_register: this.source_register == 'landing_page' ? 'Damcorp Landing Page' : 'Ads',
					name: this.form.name,
					...
					option_hear: this.form.option_hear,
				...
				if(response.data.ec == 200){					
					if(this.source_register!=''){
						this.$router.push('/success-registration')
						$cookies.remove('source_register')
						// this.$swal({
						// 	title: this.$t('success'),
						// 	text: 'Thank you for registering.<br>Our team will contact you immediately during office hour from Monday to Friday, at 9 AM - 5.30 PM.<br><br>Damcorp',
						// 	type: 'success',						
						// }).then(confirm => {
						// 	this.$router.push('login')
						// })
	...
	mounted() {
		if(this.source_register!=''){
			$cookies.set('source_register', this.source_register)
		} else {
			$cookies.remove('source_register')
		}
		// console.log('source register',this.source_register)
